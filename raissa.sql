...
-- Nome do aluno: Raissa Pereira Miranda
...
SELECT
	FORMAT(MAX(ValorUnitario), 'C', 'pt-BR') AS 'Maior Preço (R$)',
	FORMAT(MIN(ValorUnitario), 'C', 'pt-BR') AS 'Menor Preço (R$)' ------------------------------------------------------------------------------------------------------------------------
FROM PRODUTOS
...
SELECT
	P.NumeroPedido   AS 'Número do Pedido',
	PD.CodigoProduto AS 'Código do Produto',
	I.Quantidade     AS 'Quantidade de Itens'
FROM PEDIDOS P
INNER JOIN ITEM_DE_PEDIDO I ON I.NumeroPedido = P.NumeroPedido
INNER JOIN PRODUTOS PD ON PD.CodigoProduto = I.CodigoProduto
WHERE I.Quantidade = 35;
GO
...
SELECT
	...
	Salario		   AS 'Salário do Vendedor'
FROM VENDEDORES
WHERE UPPER(NomeVendedor) LIKE 'A%' OR
	  UPPER(NomeVendedor) LIKE 'C%' OR
	  UPPER(NomeVendedor) LIKE 'F%' OR
	  UPPER(NomeVendedor) LIKE 'M%'
...
SELECT
	I.NumeroPedido      AS 'Número do Pedido',
	COUNT(I.Quantidade) AS 'Quantidade Total de Produtos Diferentes nos Pedidos',
	SUM(I.Quantidade)   AS 'Total Geral de Itens em cada Pedido'
FROM ITEM_DE_PEDIDO I
GROUP BY I.NumeroPedido
ORDER BY I.NumeroPedido;
GO
...
148 = 5 pedidos
5 produtos = total de produtos diferentes
8 + 7 + 3 + 10 + 30 = total geral de itens
*/
...
SELECT 
    V.CodigoVendedor AS 'Código do Vendedor',
    V.NomeVendedor   AS 'Nome do Vendedor',
    V.Salario        AS 'Salário do Vendedor'
FROM VENDEDORES V
WHERE Salario > (
...
SELECT
	...
	I.CodigoProduto AS 'Produto do Cliente',
	P.ValorUnitario AS 'Valor Unitário do Produto',
	I.Quantidade    AS 'Quantidade de Produto',
	P.ValorUnitario * P.UnidadeProduto AS 'Valor Total a ser Pago',
	PE.PrazoEntrega AS 'Prazo de Entrega do Pedido',
	V.NomeVendedor  AS 'Nome do Vendedor'
FROM CLIENTES C
JOIN PEDIDOS PE ON C.CodigoCliente = PE.CodigoCliente
JOIN PEDIDOS PE ON V.CodigoVendedor = PE.CodigoCliente
JOIN ITEM_DE_PEDIDO I ON P.Numero_Fatura = I.CodigoProduto
JOIN PRODUTOS P ON I.CodigoProduto = P.CodigoProduto
ORDER BY C.NomeCliente, P.NomeProduto, I.Quantidade, PE.PrazoEntrega, V.NomeVendedor;
GO

/*
pedidos tem codigo do cliente
itens tem numero do pedido e codigo do produto
*/
